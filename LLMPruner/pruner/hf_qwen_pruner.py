...
# Hapus import spesifik LlamaRotaryEmbedding jika tidak dipakai, atau biarkan saja
...
class HFRMSNormPrunner(BasePruningFunc):
    ...
    def get_out_channels(self, layer):
        # [FIX] RMSNorm pasti punya weight dengan shape (hidden_size,)
        if hasattr(layer, 'weight'):
            return layer.weight.shape[0]
        # Fallback jika ada atribut hidden_size
...
class HFAttentionPrunner(BasePruningFunc):
    ...
    def prune_out_channels(self, layer: nn.Module, idxs: Sequence[int]) -> nn.Module:
        # Validasi grouping head
        # assert len(idxs) % layer.num_heads == 0 # Disable assert ini untuk Qwen GQA agar lebih fleksibel
    ...
    def get_out_channels(self, layer):
        # [FIX] Prioritaskan cek shape weight q_proj karena hidden_size kadang hilang di Qwen2
        if hasattr(layer, 'q_proj') and hasattr(layer.q_proj, 'weight'):
...
class HFLinearPrunner(BasePruningFunc):
    ...
    def prune_out_channels(self, layer: nn.Module, idxs: Sequence[int]) -> nn.Module:
        ...
        # Logika rekonstruksi weight sederhana (menghindari crash pada channel kosong)
        if keep_weight.shape[0] > 0 and remove_weight.shape[0] > 0:
        ...
        if layer.bias is not None:
            ...
            if keep_bias.shape[0] > 0 and remove_bias.shape[0] > 0:
    ...
    def prune_in_channels(self, layer: nn.Module, idxs: Sequence[int]) -> nn.Module:
        ...
        if keep_weight.shape[1] > 0 and remove_weight.shape[1] > 0:
...
##############################
# Importance (Tetap sama seperti original Anda)
##############################
